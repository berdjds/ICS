...
export function Scene() {
  ...
  const { shapeTargets, cubesData, yOffsets, awsEndIndex2 } = useMemo(() => {
    ...
    const footerPoints = Array(PARTICLE_COUNT)
      .fill(null)
      .map(() => new THREE.Vector3(0, 0, 0));
  ...
  const {
    ...
    initialParticleSizes,
    initialParticleColors,
  } = useMemo(() => {
  ...
  useFrame((state) => {
    ...
    const leftPosition = -viewport.width * 0.2; // Center of left 65% area
    ...
    mainShapeGroupRef.current.position.x = THREE.MathUtils.lerp(
      sourceX,
      nextX,
      segmentProgress
    );
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      if (currentSegment === 0) {
        const worldPos = tempVec
          .clone()
          .applyMatrix4(mainShapeGroupRef.current.matrixWorld);
        ...
        if (influence > 0) {
          ...
          const currentColor = aiSkullBaseColor
            .clone()
          ...
          const swirlDirection = new THREE.Vector3(
            mouseDirection.x * Math.cos(swirlAngle) -
              mouseDirection.y * Math.sin(swirlAngle),
            mouseDirection.x * Math.sin(swirlAngle) +
              mouseDirection.y * Math.cos(swirlAngle),
            mouseDirection.z
          );
          ...
          const floatOffsetX =
            Math.cos(time * 2.5 + i * 0.15) * influence * 0.05;
        ...
        if (currentSegment === 1) {
          ...
          if (influence > 0) {
            ...
            const currentColor = earthBaseColor
              .clone()
          ...
          const currentColor = baseColor
            .clone()
            .lerp(brightColor, influence * 0.8);
          colors[i * 3] = currentColor.r;
          colors[i * 3 + 1] = currentColor.g;
          colors[i * 3 + 2] = currentColor.b;
      ...
      if (currentSegment === 1) {
        ...
        if (combinedInfluence < 0.5) {
          colorMix = serverColorLight
            .clone()
            .lerp(serverColorBase, combinedInfluence * 2);
        } else {
          colorMix = serverColorBase
            .clone()
            .lerp(serverColorDark, (combinedInfluence - 0.5) * 2);
        }
